...
def format_baseball_innings(val):
    """
    Converts a raw cumulative float (like 57.7) into standard baseball 
    notation (57.2) where the decimal represents thirds/outs (0, 1, or 2).
    Returns an empty string if the value is 0, NaN, or None.
    """
    import pandas as pd
    import math
    ...
    # If a number like 57.7 was generated by adding raw inputs, convert it to outs:
    if decimal_part >= 0.3:
        # Extract the literal digit after the decimal (e.g., 7 outs)
        literal_outs = round(decimal_part * 10)
        total_outs = (whole_part * 3) + literal_outs
    else:
        # Standard fraction conversion (e.g., 57.3333 -> 57 and 1 out)
    ...
    # If there are 0 leftover outs, show it as a clean whole integer
    if leftover_outs == 0:
