...
with tab_team_standard_batting:
    st.dataframe(
        ...
        column_order=["season","games","at_bats","plate_appearances","hits","singles","doubles","triples","home_runs","total_bases","player_runs","runs_batted_in","walks","strikeouts_batting",
                     "sacrifice_flies","batting_double_plays","batting_average"],
        column_config={
            "season": st.column_config.Column("Season", pinned=True, help="**Season**"),
            "games": st.column_config.NumberColumn("G", format="%d", help="**Games**"),
            ...
            "player_runs": st.column_config.NumberColumn("R", format="%d", help=gloss_r),
...
with tab_team_advanced_batting:
    st.dataframe(
        ...
        column_config={
            ...
            "team_calculated_war": st.column_config.NumberColumn("WAR", format="%.1f", help=gloss_war)
        }
    )
...
if tab_box_scores.open:
    with tab_box_scores:
        ...
        # Check for meta data
        has_inning_data = pd.notna(game_meta.get("innings")) and game_meta["innings"] != ""
        ...
        with tab_box_score_batting:
            st.dataframe(
                ...
                column_config={
                    ...
                    "slugging_percentage": st.column_config.NumberColumn("SLG", format="%.3f", help=gloss_slg),
                    "on_base_plus_slugging": st.column_config.NumberColumn("OPS", format="%.3f", help=gloss_ops),
                    "wrc": st.column_config.NumberColumn("wRC", format="%d", help=gloss_wrc),
                    "woba": st.column_config.NumberColumn("wOBA", format="%.3f", help=gloss_woba),
                    "wrc_plus": st.column_config.NumberColumn("wRC+", format="%d", help=gloss_wrc_plus)
                }
            )
